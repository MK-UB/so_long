...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   maping_setup_bonus.c                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: melayoub <melayoub@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/02/20 17:52:43 by melayoub          #+#    #+#             */
/*   Updated: 2023/02/24 14:16:02 by melayoub         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	setup_xpm(t_list *content)
{
	...
	content->plancton
		= mlx_xpm_file_to_image(content->mlx_ptr, "./xp/plancton.xpm", &y, &y);
}
...
void	setup_imgs(t_list *content)
{
	...
	content->win_ptr
		= mlx_new_window(content->mlx_ptr, width, height, "so_long_bonus");
	if (!content->win_ptr)
	...
	if (!content->bob_down || !content->bob_up || !content->bobleft
		...
		|| !content->jellyfish || !content->safe || !content->wall
		|| !content->water)
